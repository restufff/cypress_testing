...
it.only.describe("Edit Data Test", () => {
  it("Edit Data", () => {
    cy.get(
      ":nth-child(1) > :nth-child(6) > .table_actionContainer__2htuk > .edit"
    ).click();

    cy.get(".form_gridInput__1LuaA > :nth-child(1) > input").clear();
    cy.get(".form_gridInput__1LuaA > :nth-child(1) > input").type("Lele");

    cy.get(".form_gridInput__1LuaA > :nth-child(2) > input").clear();
    cy.get(".form_gridInput__1LuaA > :nth-child(2) > input").type("50");

    cy.get(":nth-child(3) > input").clear();
    cy.get(":nth-child(3) > input").type("Bengkulu");

    cy.get(":nth-child(4) > input").clear();
    cy.get(":nth-child(4) > input").type("Kepahiang");

    cy.get(":nth-child(5) > input").clear();
    cy.get(":nth-child(5) > input").type("50000");
